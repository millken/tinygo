...
// https://github.com/go-delve/delve/blob/v1.23.1/pkg/elfwriter/writer.go
package elf
...
func New(w WriteCloserSeeker, fhdr *FileHeader) *Writer {
	const (
		ehsize    = 64
		phentsize = 56
		shentsize = 64
	...
	if fhdr.Class != ELFCLASS64 {
		panic("unsupported")
	}

	if fhdr.Data != ELFDATA2LSB {
		panic("unsupported")
	}
	...
	r.u16(uint16(SHN_UNDEF)) // e_shstrndx
...
// notes.
func (w *Writer) WriteNotes(notes []Note) *ProgHeader {
	if len(notes) == 0 {
		return nil
	}
	h := &ProgHeader{
